# Ecommerce Product Blocks — Implementation Plan (Phase 1)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Convert 5 ecommerce product blocks (ProductCard, ProductGrid, FeaturedProduct, ProductCarousel, QuickView) from hardcoded ECOMMERCE_DEFS to proper widgets with full render() and editPanel().

**Architecture:** New `widgets/ecom-products.js` file registers all 5 as `FB.widgets.register()` with `category: "ecommerce"`. Each has its own `render()` (moved from canvas.js switch-case with XSS escaping added) and `editPanel()` (all props editable). Old entries removed from `blocks.js` ECOMMERCE_DEFS and `canvas.js` switch-case.

**Tech Stack:** Vanilla JS, existing FB global namespace, same pattern as widgets/icon-box.js.
...
| Action | File                       | Details                                       |
| ------ | -------------------------- | --------------------------------------------- |
| Create | `widgets/ecom-products.js` | All 5 widget registrations (~500 lines)       |
| Modify | `blocks.js:1216-1699`      | Remove 5 migrated entries from ECOMMERCE_DEFS |
| Modify | `canvas.js:2454-2691`      | Remove 5 migrated switch-case entries         |
| Modify | `src/main.js`              | Add import for ecom-products.js               |

---

### Task 1: Create widgets/ecom-products.js

**Files:**

- Create: `widgets/ecom-products.js`

- [ ] **Step 1: Write the complete ecom-products.js file**
...
```bash
node --check widgets/ecom-products.js
```
...
### Task 2: Remove migrated blocks from ECOMMERCE_DEFS

**Files:**

- Modify: `blocks.js:1216-1430` (remove 5 product block entries)

- [ ] **Step 1: Remove 5 entries from ECOMMERCE_DEFS**

In `/home/shauntuhey/Framework/js/blocks.js`, remove these 5 entries from `FB.blocks.ECOMMERCE_DEFS`:

1. `ecomProductCard` (lines 1217-1238)
2. `ecomProductGrid` (lines 1239-1309)
3. `ecomFeaturedProduct` (lines 1310-1331)
4. `ecomProductCarousel` (lines 1332-1403)
5. `ecomQuickView` (lines 1404-1426)

Replace them with a comment:

```js
// ecomProductCard, ecomProductGrid, ecomFeaturedProduct,
// ecomProductCarousel, ecomQuickView moved to widgets/ecom-products.js
```

The `ECOMMERCE_DEFS` object remains (still has 13 other blocks for Phase 2/3).

- [ ] **Step 2: Verify structure preserved**

Make sure the remaining 13 entries still form a valid object assigned to `FB.blocks.ECOMMERCE_DEFS`.

---

### Task 3: Remove migrated switch-case entries from canvas.js

**Files:**

- Modify: `canvas.js:2454-2691`

- [ ] **Step 1: Remove 5 case entries from renderBlockHTML**

In `/home/shauntuhey/Framework/js/canvas.js`, remove these 5 case blocks:

1. `case "ecomProductCard":` (lines 2454-2503)
2. `case "ecomProductGrid":` (lines 2505-2560)
3. `case "ecomFeaturedProduct":` (lines 2562-2602)
4. `case "ecomProductCarousel":` (lines 2605-2642)
5. `case "ecomQuickView":` (lines 2644-2691)
...
The switch-case must remain valid (other ecom cases still live here).
...
- [ ] **Step 1: Add import for ecom-products.js**

Add the following line after the existing widget imports in `/home/shauntuhey/Framework/src/main.js`:
...
1. **Spec coverage:** All 5 product blocks from the spec are covered (Task 1). Each has full editPanel for every prop. Spec behavior (no image state, sale price rendering, badges, stars) is implemented. ✓
2. **Placeholder scan:** No TBDs, TODOs, or incomplete code. All render functions have complete HTML with XSS escaping. All edit panels have complete controls. ✓
3. **Type consistency:** Widget type strings match between blocks.js removal (`ecomProductCard`, etc.) and widget registration names. Props match between defaultProps and editPanels. ✓
4. **Category:** All 5 use `category: "ecommerce"` for consistent palette grouping. ✓
